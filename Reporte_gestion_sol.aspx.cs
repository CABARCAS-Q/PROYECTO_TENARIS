...
{
    public partial class Reporte_gestion_sol : System.Web.UI.Page
    {

        Consultas objetoconexiones = new Consultas();
        private static MyLogClass Log = new MyLogClass("Reporte_gestion_sol.aspx");
                ...
                try
                {
                    ...
                    if (re.Count > 0)
                    {
                        ...
                        foreach (Object obj in re)
                        {
                            ...
                            if (valor[4] == "Reporte_gestion_sol.aspx")
                            {
                                validaventana = 1;
                            }
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string lista_motivoss()
        {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string lista_solicitudes(string fecha1, string fecha2, string linea, string maquina, string estado)
        {
            ...
            try
            {
                ...
                if (estado != "0")
                {
                    parametros.Add("tipo", estado);
                }
              

                Consultas objetoconexiones = new Consultas();
                dt = objetoconexiones.metodolista_datos_d1("[ANFA].[Reporte_solicitudes_personal]", parametros);
            }
            catch (Exception er)
            {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string lista_solicitudes_lineas(string fecha1, string fecha2, string usuarioh, string estado)
        {
            ...
            try
            {
                ...
                {
                    parametros.Add("Usuarioh", usuarioh);
                }
                ...
                Consultas objetoconexiones = new Consultas();
                dt = objetoconexiones.metodolista_datos_d1("[ANFA].[Reporte_solicitudes_lineas]", parametros);
            }
            catch (Exception er)
            {
