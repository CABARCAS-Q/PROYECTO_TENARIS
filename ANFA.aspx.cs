...
{
    public partial class ANFA : System.Web.UI.Page
    {
        Consultas objetoconexiones = new Consultas();
        private static MyLogClass Log = new MyLogClass("ANFA.aspx");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!this.IsPostBack) // to avoid reloading your control on postback
            {
                ...
                try
                {
                    ...
                    {
                        Log.write(MyLogClass.LogLevel.Info, "ANFA:parametro1:" + Request.QueryString["parametro1"].ToString());
                        bidticket.Value = Request.QueryString["parametro1"].ToString();
                    }
                ...
                fechasistema.Value = DateTime.Now.ToString();
        ...
        public static string Max_anfa()
        {
            ...
            try
            {
                ...
                Consultas objetoconexiones = new Consultas();
                dt = objetoconexiones.metodolista_datos("[ANFA].[Max_anfa]");
            }
            catch (Exception er)
            {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string Lista_solicitudes()
        {
            ...
            try
            {
                ...
                Consultas objetoconexiones = new Consultas();
                dt = objetoconexiones.metodolista_datos("[Helpdesk].[Todas_solicitudes]");
            }
            catch (Exception er)
            {
            ...
            {
                data.Add(row["id_solicitud"].ToString());
            }
            return JsonConvert.SerializeObject(new { Error = error, Mensaje_error = error_mensaje, Resul = data.ToArray() });
        }
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string Crear_anfa(string idticket, string personal, string quepaso, string causa, string solucion, string analisis, string acciones)
        {
            ...
            try
            {
                string archivos = HttpContext.Current.Session["Archivos"].ToString();

                Dictionary<string, Object> parametros = new Dictionary<string, object>();
                parametros.Add("Usuario", usuario);
                parametros.Add("Idticket", idticket);
                parametros.Add("Personal", personal);
                parametros.Add("Quepaso", quepaso);
                parametros.Add("Causa", causa);
                parametros.Add("Solucion", solucion);
                parametros.Add("Analisis", analisis);
                parametros.Add("Acciones", acciones);
                parametros.Add("Archivos", archivos);

                Consultas objetoconexiones = new Consultas();
                ArrayList re = objetoconexiones.metodolista_datos1("[ANFA].[Generar_nuevo_anfa]", parametros);
                if (re.Count > 0)
                {
                    ...
                    foreach (Object obj in re)
                    {
                        ...
                        {
                            valor = "1";
                            HttpContext.Current.Session["Archivos"] = "";
                        }
                        else if (valores[0] == "-2")
                        {
                            valor = "-2";
        ...
        protected void UploadComplete(object sender, AjaxControlToolkit.AjaxFileUploadEventArgs e)
        {
            ...
            try
            {
                ...
                if (Session["Archivos"].ToString().Length > 1)
                {
                    Session["Archivos"] = Session["Archivos"] + "|" + fileName.ToString();
                }
                else
                {
                    Session["Archivos"] = fileName.ToString();
                }
