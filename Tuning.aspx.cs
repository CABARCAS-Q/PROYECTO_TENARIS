...
{
    public partial class Tuning : System.Web.UI.Page
    {
        Consultas objetoconexiones = new Consultas();
        private static MyLogClass Log = new MyLogClass("Turning.aspx");
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string crear_solicitud(string fecha, string usuarios, string observaciones)
        {
            ...
            try
            {
                ...
                Debug.WriteLine("DATOS:" + fecha);
                Debug.WriteLine("DATOS:" + usuarios);

                Dictionary<string, Object> parametros = new Dictionary<string, object>();
                parametros.Add("Fechas", fecha);
                parametros.Add("Usuarios", usuarios);
                parametros.Add("Usuario", usuario);
                parametros.Add("Observaciones", observaciones);

                Consultas objetoconexiones = new Consultas();
                ArrayList re = objetoconexiones.metodolista_datos1("[helpdesk].[Insert_history_programacion]", parametros);
                if (re.Count > 0)
                {
                    ...
                    foreach (Object obj in re)
                    {
                        ...
                        {
                            error_mensaje = "Existen programaciones cargadas revisar programaciones";
                            error = "-4";
                            valor = "-4";
                        }
                        else
