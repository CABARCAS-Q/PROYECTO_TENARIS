...
namespace Helpdesk_Auto.Helpdesk
{
    public partial class Config_usuarios : System.Web.UI.Page
    {
        ...
        private static MyLogClass Log = new MyLogClass("Config_usuarios.aspx");
            ...
            try
            {
                ...
                if (re.Count > 0)
                {
                    ...
                    foreach (Object obj in re)
                    {
                        ...
                        if (valor[4] == "Config_usuarios.aspx")
                        {
                            validaventana = 1;
                        }
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string crear_usuario(string id,string nombre,string apellidos,string email,string perfil,string estado,string tipo)
        {
            ...
            try
            {
                ...
                Consultas objetoconexiones = new Consultas();
                ArrayList re = objetoconexiones.metodolista_datos1("[Sis_config].[Insertar_usuario_sistema]", parametros);
                if (re.Count > 0)
                {
                    ...
                    foreach (Object obj in re)
                    {
                        ...
                        {
                            error_mensaje = "ya hay un usuario con este mismo id,si desea actualizar escoga la opcion actualizar";
                            error = "-2";
                            valor = "-2";
                        }
                        else if (valores[0] == "-3")
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string lista_usuarios(string fid, string fnombre, string fperfil, string festado)
        {
            ...
            try
            {
                ...
                if (fid.Length > 0)
                {
                    parametros.Add("id", fid);
                }
                if (fnombre.Length > 0)
                {
                    parametros.Add("nombre", fnombre);
                }
                if (fperfil !="0")
                {
                    parametros.Add("perfil", fperfil);
                }
                if (festado !="Todos")
                {
                    parametros.Add("Estado", festado);
                }
               

                Consultas objetoconexiones = new Consultas();
                dt = objetoconexiones.metodolista_datos_d1("[Sis_config].[Lista_usuarios_sistemas]", parametros);
            }
            catch (Exception er)
            {
