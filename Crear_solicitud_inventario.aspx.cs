...
{
    public partial class Crear_solicitud_inventario : System.Web.UI.Page
    {
        Consultas objetoconexiones = new Consultas();
        private static MyLogClass Log = new MyLogClass("Crear_solicitud_inventario.aspx");
        ...
        public static string Max_solicitud()
        {
            ...
            try
            {
                ...
                Consultas objetoconexiones = new Consultas();
                dt = objetoconexiones.metodolista_datos("[Inventory].[Max_solicitud]");
            }
            catch (Exception er)
            {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string crear_solicitud(string maquina, string observaciones, string tipos)
        {
            ...
            try
            {
                ...
                Debug.WriteLine("DATOS:" + tipos);

                Dictionary<string, Object> parametros = new Dictionary<string, object>();
                parametros.Add("Usuario", usuario);
                parametros.Add("idmachine", maquina);
                parametros.Add("Tipos", tipos);
                parametros.Add("Observaciones", observaciones);


                Consultas objetoconexiones = new Consultas();
                ArrayList re = objetoconexiones.metodolista_datos1("[Inventory].[Insert_history]", parametros);
                if (re.Count > 0)
                {
